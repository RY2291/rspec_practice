  ...
  describe "入力項目の有無" do
    context "必須入力項目であること" do
      ...
      it "年齢が必須であること" do
        new_enquete = FoodEnquete.new
        expect(new_enquete).not_to be_valid
        expect(new_enquete.errors[:age]).to include(I18n.t('errors.messages.blank'))
      end

      it "満足度が必須であること" do
        new_enquete = FoodEnquete.new
        expect(new_enquete).not_to be_valid
        expect(new_enquete.errors[:score]).to include(I18n.t('errors.messages.blank'))
      end
    ...
    context "任意入力であること" do
      it "ご意見・ご要望が任意であること" do
        new_enquete = FoodEnquete.new
        expect(new_enquete).to be_invalid
